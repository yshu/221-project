...
class config():
    ...
    output_path        = "results/pong_resnet_3/"
...
def prelu(inputs):
    return tf.contrib.keras.layers.PReLU(shared_axes=[1,2])(inputs)

def batch_norm(inputs, is_training):
    return layers.batch_norm(inputs, fused=True, is_training=is_training)

def residual_block1(X, is_training, n=64, s=1):
    skip = layers.conv2d(X, n*2, 1, 2, activation_fn=None)
    skip = batch_norm(skip, is_training)

    conv1 = layers.conv2d(X, n, 3, s, activation_fn=None)
    bn1 = batch_norm(conv1, is_training)
    prelu1 = prelu(bn1)
    conv2 = layers.conv2d(prelu1, n*2, 3, 1, activation_fn=None)
    bn2 = batch_norm(conv2, is_training)
    out = bn2 + skip
    return out

def residual_block2(X, is_training, n=64, s=1):
    conv1 = layers.conv2d(X, n, 3, 1, activation_fn=None, biases_initializer=None)
    bn1 = batch_norm(conv1, is_training)
    prelu1 = prelu(bn1)
    conv2 = layers.conv2d(prelu1, n, 3, 1, activation_fn=None, biases_initializer=None)
    bn2 = batch_norm(conv2, is_training)
    out = bn2 + X
    return out
...
class ResNet(object):
    ...
    def q_network_op(self, state):
        ...
        conv = residual_block1(conv, self.config.is_training, 32, 2)
        for i in range(3):
            conv = residual_block2(conv, self.config.is_training, 64, 1)
            
        skip = layers.conv2d(skip, 64, 1, 2, activation_fn=None)
        skip = batch_norm(skip, self.config.is_training)
        conv = conv + skip
...
class train_ResNet(ResNet):
    def __init__(self, env, config):
        ResNet.__init__(self, env, config)
    ...
    def run(self, exp_schedule, lr_schedule):
        ...
        self.saver.restore(self.sess, 'results/pong_resnet_3/model.ckpt-5000000')
