...
class config():
    ...
    record             = True
    output_path        = "results/test_dueling_TL_2/"
    model_output       = output_path + "model.ckpt-5000000"
    model_output2      = output_path + "model2.ckpt"
...
class Dueling(object):
    ...
    def q_network_op(self, state):
        """
        Google's Dueling Network paper
        ...
        out1 = layers.fully_connected(out, 1, activation_fn=None)
        out2 = layers.fully_connected(out, num_actions, activation_fn=None)
        out2 = out2 - tf.tile(tf.reduce_mean(out2, axis=1, keepdims=True), [1, num_actions])
        out = tf.tile(out1, [1, num_actions]) + out2
        return out
    ...
    def build(self):
        ...
        q_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='q')
        # only update fully connected layers
        retrain_vars = [var for var in q_vars if 'fully' in var.name]
        grads_and_vars = optimizer.compute_gradients(self.loss, retrain_vars)
        self.train_op = optimizer.apply_gradients(grads_and_vars)
        
        cnn_vars = [var for var in q_vars if 'fully' not in var.name]
        self.cnn_saver = tf.train.Saver(cnn_vars)
...
if __name__ == '__main__':
    ...
    model = train_Dueling(env, config)
